...
function renderForgotForm() {
  return `
    ...
    <div class="auth-sub">Enter your email and we'll send a reset link instantly</div>
    <div id="auth-msg"></div>
    <div class="field">
      <label class="lbl">Email Address</label>
      <input class="inp" id="a-email" type="email" placeholder="you@example.com" autocomplete="email"/>
    </div>
    <button class="btn btn-primary" id="a-reset-btn" style="width:100%;margin-bottom:14px">Send Reset Link</button>
    <div style="text-align:center;font-size:12.5px;color:var(--text-2)">
      Remembered it? <span class="auth-link" id="back-login">Back to Sign In</span>
    </div>`;
}
...
function renderLoginForm() {
  ...
  return `
    <div class="auth-title">${isLogin ? 'Welcome back' : 'Get started free'}</div>
    <div class="auth-sub">${isLogin ? 'Sign in to your damage intelligence dashboard' : 'Detect mobile damage with AI — free forever'}</div>
    <div id="auth-msg"></div>
    ...
    <div class="field">
      <label class="lbl" style="display:flex;justify-content:space-between;align-items:center">
        <span>Password</span>
        ${isLogin ? `<span class="auth-link" id="forgot-link" style="font-size:10.5px">Forgot password?</span>` : ''}
      </label>
    ...
    <div style="text-align:center;font-size:12.5px;color:var(--text-2)">
      ...
      <span class="auth-link" id="auth-toggle">${isLogin ? 'Create one free' : 'Sign in'}</span>
    </div>

    <div class="auth-footer-note">🔒 FIREBASE AUTH · END-TO-END ENCRYPTED · GDPR COMPLIANT</div>`;
}
...
function friendlyErr(code) {
  const m = {
    ...
    'auth/configuration-not-found': 'Firebase not configured. Open src/firebase/config.js and add your project keys.',
    'auth/unauthorized-domain':     'Domain not authorized. Go to Firebase Console → Authentication → Settings → Authorized Domains and add localhost.',
    'auth/internal-error':          'Firebase internal error. Check your config.js values.',
    'auth/api-key-not-valid':       'Firebase API key is invalid. Open src/firebase/config.js and paste your real keys from Firebase Console → Project Settings.',
    'auth/invalid-api-key':         'Firebase API key is invalid. Open src/firebase/config.js and paste your real keys from Firebase Console → Project Settings.',
  };
